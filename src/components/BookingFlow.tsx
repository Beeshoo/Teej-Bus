...
import { CITIES, TRIP_SCHEDULES, calculateTravelDuration, calculateTripPrice, formatArrivalTime, to12Hour } from '../constants';
  ...
  return (
    <div className="max-w-4xl mx-auto my-4 md:my-8 p-2 md:p-4">
      <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-2xl p-4 md:p-10 border border-gray-100">
          ...
          <div className="space-y-6 md:space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-2">
                <label className="text-sm md:text-base font-bold text-gray-400 me-2">من محافظة</label>
                <select className="w-full p-5 md:p-6 rounded-xl md:rounded-2xl bg-gray-50 border-2 border-transparent focus:border-amber-500 outline-none transition-all appearance-none cursor-pointer text-lg md:text-xl" value={booking.from} onChange={e => {
                  ...
                  setBooking(prev => ({ ...prev, from: newFrom, to: prev.to === newFrom ? '' : prev.to }));
                }}>
              ...
              <div className="space-y-2">
                <label className="text-sm md:text-base font-bold text-gray-400 me-2">إلى محافظة</label>
                <select className="w-full p-5 md:p-6 rounded-xl md:rounded-2xl bg-gray-50 border-2 border-transparent focus:border-amber-500 outline-none transition-all appearance-none cursor-pointer text-lg md:text-xl" value={booking.to} onChange={e => {
                  ...
                  setBooking(prev => ({ ...prev, to: newTo, from: prev.from === newTo ? '' : prev.from }));
                }}>
            ...
            <div className="space-y-2">
              <label className="text-sm md:text-base font-bold text-gray-400 me-2">تاريخ السفر</label>
            ...
            <div className="grid grid-cols-1 gap-3 md:gap-4">
               {(() => {
                 ...
                 return availableTrips.map(ts => {
                   const duration = calculateTravelDuration(booking.from, booking.to);
                   const dynamicArrivalTime = formatArrivalTime(ts.departureTime, duration);
                   return (
                    <div key={ts.id} onClick={() => setBooking({...booking, departureTime: ts.departureTime, arrivalTime: dynamicArrivalTime})} className={`p-4 md:p-6 border-2 rounded-2xl md:rounded-[2rem] cursor-pointer transition-all flex justify-between items-center group ${booking.departureTime === ts.departureTime ? 'bg-blue-50 border-blue-600 shadow-lg' : 'bg-white border-gray-100 hover:border-blue-200'}`}>
                      <div className="flex items-center gap-4 md:gap-6">
                         ...
                         <div>
                           ...
                           <p className="text-sm md:text-base text-gray-400 font-bold">وصول متوقع {to12Hour(dynamicArrivalTime)}</p>
                         </div>
                      </div>
                      <div className="text-xl md:text-2xl font-black text-blue-900">{to12Hour(ts.departureTime)}</div>
                    </div>
                   );
                 });
               })()}
          ...
          <div className="space-y-6 md:space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
               {(Object.values(BusType) as BusType[]).map(type => (
                 <div key={type} onClick={() => setBooking({...booking, busType: type, selectedSeats: []})} className={`p-6 md:p-8 border-2 rounded-[2rem] md:rounded-[2.5rem] cursor-pointer transition-all text-center relative overflow-hidden group ${booking.busType === type ? 'bg-amber-50 border-amber-500 shadow-xl scale-105' : 'bg-white border-gray-100 hover:border-amber-200'}`}>
                   ...
                   <p className="text-[10px] md:text-xs text-gray-400 font-bold mb-4 leading-relaxed h-12 flex items-center justify-center">
                     {type === BusType.STANDARD ? 'سفر مريح • أسعار منافسة • شاشات عامة' : type === BusType.VIP ? 'مقاعد واسعة • شاشات ذكية • مشروبات مجانية' : 'خصوصية تامة • مقاعد كاملة الإنحناء • وجبات ساخنة'}
                   </p>
                   <div className="text-xl md:text-2xl font-black text-amber-600">
                     {calculateTripPrice(booking.from, booking.to, type)} <span className="text-xs">ج.م</span>
                   </div>
                   {booking.busType === type && <div className="absolute top-4 left-4 text-amber-500"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg></div>}
                 </div>
               ))}
            </div>
            <div className="flex gap-3 md:gap-4">
        ...
        {step === 4 && (
          <div className="space-y-6 md:space-y-8 text-center animate-fadeIn">
             <div className="text-center">
              ...
              <p className="text-sm md:text-gray-400 mt-2 font-medium">توزيع المقاعد يختلف حسب فئة الحافلة</p>
             </div>
             <div className="relative inline-block mx-auto group/bus">
               <div className="bg-slate-50 border-[6px] border-slate-200 rounded-[3rem] md:rounded-[4rem] p-4 sm:p-8 md:p-12 shadow-2xl relative overflow-hidden">
                 ...
                 <div className="flex justify-end items-center mb-12 pb-8 border-b-2 border-slate-100">
                   <div className="flex flex-col items-center gap-2">
                     <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-2xl border-2 border-slate-200 shadow-sm relative overflow-hidden">👨‍✈️</div>
                     <div className="text-[10px] font-black text-slate-400">الكابتن</div>
                   </div>
                 </div>

                 <div className={`grid gap-y-4 md:gap-y-6 mx-auto justify-center ${booking.busType === BusType.STANDARD ? 'grid-cols-5 gap-x-2' : booking.busType === BusType.VIP ? 'grid-cols-4 gap-x-8' : 'grid-cols-3 gap-x-12 md:gap-x-20'}`}>
                   {(() => {
                     let rows = 10;
                     let layout = [1, 2, 0, 3, 4];
                     let seatsPerRow = 4;
                     if (booking.busType === BusType.VIP) { rows = 8; layout = [1, 0, 2, 3]; seatsPerRow = 3; }
                     else if (booking.busType === BusType.ROYAL) { rows = 6; layout = [1, 0, 2]; seatsPerRow = 2; }

                     return Array.from({length: rows}, (_, row) => {
                       return layout.map((col, colIdx) => {
                         if (col === 0) return <div key={`aisle-${row}`} className="flex items-center justify-center"><div className="w-0.5 h-full bg-slate-200 rounded-full min-h-[40px]"></div></div>;
                         const seatNumber = row * seatsPerRow + col;
                         const isOccupied = occupiedSeats.includes(seatNumber);
                         const isSelected = booking.selectedSeats.includes(seatNumber);
                         return (
                           <button key={`seat-${seatNumber}`} disabled={isOccupied} onClick={() => toggleSeat(seatNumber)} className={`w-10 h-10 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-xl border-2 font-black transition-all flex flex-col items-center justify-center relative ${isOccupied ? 'bg-slate-100 text-slate-300 cursor-not-allowed border-slate-100' : isSelected ? 'bg-blue-900 border-blue-900 text-white shadow-xl scale-110 z-10' : 'bg-white border-slate-200 text-slate-500 hover:border-blue-500 hover:shadow-lg'}`}>
                             <div className={`absolute -top-1 w-1/2 h-1.5 rounded-t-lg ${isSelected ? 'bg-blue-800' : isOccupied ? 'bg-slate-200' : 'bg-slate-100'}`}></div>
                             <span className="text-[10px] md:text-sm">{seatNumber}</span>
                           </button>
                         );
                       });
                     });
                 ...
                 <div className="mt-12 pt-8 border-t-2 border-slate-100 flex justify-center"><div className="w-24 h-2 bg-slate-200 rounded-full"></div></div>
               </div>
             </div>
             
             <div className="flex justify-center gap-4 text-sm font-bold bg-white p-4 rounded-2xl border border-gray-100 max-w-fit mx-auto">
               ...
               <div className="flex items-center gap-2"><div className="w-4 h-4 bg-slate-100 rounded-md"></div> محجوز</div>
             </div>

             <div className="flex gap-3 md:gap-4 mt-8">
               <button onClick={handleBack} className="flex-1 bg-gray-100 py-5 rounded-2xl font-black text-gray-500">رجوع</button>
               <button onClick={handleNext} disabled={!canGoNext()} className="flex-[2] bg-blue-900 text-white py-5 rounded-2xl font-black shadow-xl disabled:opacity-30 transition-all">
        ...
        {step === 5 && (
          <PaymentForm 
            amount={booking.selectedSeats.length * calculateTripPrice(booking.from, booking.to, booking.busType)} 
