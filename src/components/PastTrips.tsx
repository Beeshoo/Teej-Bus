...
const PastTrips: React.FC<PastTripsProps> = ({ tickets, onRate, onMarkComplete, onCancel, singleTicketId }) => {
  ...
  const [downloadingTicket, setDownloadingTicket] = useState<Ticket | null>(null);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = image;
      link.download = `TajBus-Ticket-${downloadingTicket?.id}.png`;
      link.click();
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 md:py-12 px-4 animate-fadeIn">
      ...
      {/* Smart Ticket Preview Modal */}
      {downloadingTicket && (
        <div className="fixed inset-0 bg-blue-950/90 backdrop-blur-md z-[150] flex items-center justify-center p-4 overflow-y-auto">
          <div className="max-w-xl w-full">
            <div className="flex justify-between items-center mb-6 px-4">
              <h3 className="text-xl font-black text-white">تذكرة الرحلة الذكية</h3>
              <button onClick={() => setDownloadingTicket(null)} className="text-white bg-white/10 p-2 rounded-full hover:bg-white/20 transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18 6L6 18M6 6l12 12"></path></svg>
              </button>
            </div>

            {/* Visual Boarding Pass Container */}
            <div ref={ticketRef} className="bg-white rounded-[2.5rem] shadow-2xl relative overflow-hidden font-tajawal" dir="rtl">
              {/* Ticket Header */}
              <div className="bg-blue-900 p-8 text-white relative">
                <div className="absolute top-[-50px] right-[-50px] w-64 h-64 bg-amber-500/10 rounded-full blur-3xl"></div>
                <div className="relative z-10 flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                       <div className="w-8 h-8 bg-amber-500 rounded-lg flex items-center justify-center text-blue-900 font-black">T</div>
                       <p className="text-xl font-black">تاج باص - Tej Bus</p>
                    </div>
                    <p className="text-xs font-bold text-blue-300 uppercase tracking-widest leading-none">Global Booking Standard</p>
                  </div>
                  <div className="text-left">
                    <p className="text-xs font-bold text-blue-400">رقم التذكرة</p>
                    <p className="text-xl font-black">#{downloadingTicket.id}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 pb-12">
                <div className="flex justify-between items-center mb-10">
                   <div className="text-right">
                      <p className="text-4xl font-black text-blue-900 leading-none mb-2">{downloadingTicket.from}</p>
                      <p className="text-sm font-bold text-gray-400">محطة الإركاب</p>
                   </div>
                   <div className="flex-grow flex flex-col items-center px-8 opacity-40">
                      <div className="text-2xl mb-1">🚌</div>
                      <div className="w-full h-[2px] border-b-2 border-dashed border-blue-900"></div>
                   </div>
                   <div className="text-left">
                      <p className="text-4xl font-black text-blue-900 leading-none mb-2">{downloadingTicket.to}</p>
                      <p className="text-sm font-bold text-gray-400">محطة الوصول</p>
                   </div>
                </div>

                <div className="grid grid-cols-2 gap-y-8 gap-x-4 mb-10">
                   <div>
                     <p className="text-xs font-bold text-gray-400 mb-1">تاريخ الرحلة</p>
                     <p className="text-xl font-black text-blue-900">{downloadingTicket.date}</p>
                   </div>
                   <div>
                     <p className="text-xs font-bold text-gray-400 mb-1">وقت التحرك</p>
                     <p className="text-xl font-black text-blue-900">{to12Hour(downloadingTicket.departureTime)}</p>
                   </div>
                   <div>
                     <p className="text-xs font-bold text-gray-400 mb-1">المقاعد المحجوزة</p>
                     <p className="text-xl font-black text-blue-900">{downloadingTicket.selectedSeats.join(', ')}</p>
                   </div>
                   <div>
                     <p className="text-xs font-bold text-gray-400 mb-1">فئة الرحلة</p>
                     <p className="text-xl font-black text-amber-600">{downloadingTicket.busType}</p>
                   </div>
                </div>

                {/* Dashed Line */}
                <div className="relative py-4 mb-8">
                   <div className="absolute left-[-32px] top-1/2 -translate-y-1/2 w-8 h-8 bg-blue-950 rounded-full z-20"></div>
                   <div className="absolute right-[-32px] top-1/2 -translate-y-1/2 w-8 h-8 bg-blue-950 rounded-full z-20"></div>
                   <div className="w-full h-0 border-t-2 border-dashed border-gray-100 relative"></div>
                </div>

                <div className="flex justify-between items-end">
                   <div>
                      <p className="text-xs font-bold text-gray-400 mb-2">معلومات الكابتن</p>
                      <div className="flex items-center gap-3 bg-gray-50 p-3 pe-2 ps-4 rounded-2xl border border-gray-100">
                         <div className="w-10 h-10 bg-blue-900 rounded-xl flex items-center justify-center overflow-hidden">
                            {downloadingTicket.driverPhoto ? (
                              <img src={downloadingTicket.driverPhoto} alt={downloadingTicket.driverName} className="w-full h-full object-cover" crossOrigin="anonymous" referrerPolicy="no-referrer" />
                            ) : (
                              <span className="text-xl">👨‍✈️</span>
                            )}
                         </div>
                         <div>
                            <p className="text-sm font-black text-blue-900">{downloadingTicket.driverName || 'جاري التعيين...'}</p>
                            <p className="text-[10px] font-bold text-gray-400 uppercase">DRIVER IN CHARGE</p>
                         </div>
                      </div>
                   </div>
                   <div className="flex flex-col items-center">
                      <div className="bg-white p-2 rounded-2xl shadow-xl border border-gray-50 mb-2">
                        <QRCodeSVG 
                          value={`https://tajbus.com/verify/${downloadingTicket.id}`}
                          size={100}
                          level="H"
                          includeMargin={false}
                        />
                      </div>
                      <p className="text-[10px] font-black text-blue-900 tracking-widest">المسح للتحقق</p>
                   </div>
                </div>
              </div>
            ...
            <div className="mt-8 flex gap-4">
              <button 
                onClick={() => setDownloadingTicket(null)} 
                disabled={isDownloading}
                className="flex-1 py-4 bg-white/10 text-white rounded-2xl font-black hover:bg-white/20 transition-all disabled:opacity-30"
              >
                إغلاق
              </button>
              <button 
                ...
                disabled={isDownloading}
                className="flex-[2] py-4 bg-amber-500 text-blue-950 rounded-2xl font-black shadow-xl shadow-amber-500/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-3 disabled:opacity-70 group"
              >
                {isDownloading ? (
              ...
              <div className="p-6 md:p-8 flex-grow">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                   ...
                   <div className="w-full sm:w-auto text-center flex flex-col sm:flex-row gap-2">
                     <button 
                       onClick={() => setDownloadingTicket(t)}
                       className="w-full sm:w-auto bg-amber-500 text-blue-950 px-6 py-2 rounded-xl text-xs md:text-sm font-black shadow-lg shadow-amber-500/20 hover:scale-105 transition-all flex items-center justify-center gap-2"
                     >
                       <span>🎫</span> التذكرة الذكية
                     </button>
                    {t.status === 'upcoming' ? (
                      <>
                        <button 
